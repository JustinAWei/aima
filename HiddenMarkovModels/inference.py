        ...
        for oldPos in self.allPositions:
            if self.beliefs[oldPos] > 0:
                ...
                for newPos, prob in newPosDist.items():
                    temp[newPos] += prob * self.beliefs[oldPos]
        self.beliefs = temp
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        u = int(n / self.numParticles)
        leftover = n % self.numParticles
        self.particles = u * self.allPositions
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for g in self.legalPositions:
            beliefDist[g] *= self.getObservationProb(observation, p, g, j)
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.legalPositions:
            if beliefDist[oldPos] == 0:
                continue
            newPosDist = self.getPositionDistribution(gameState, oldPos)
            for newPos, prob in newPosDist.items():
                newBeliefDist[newPos] += beliefDist[oldPos] * prob
        self.particles = [newBeliefDist.sample() for _ in range(self.numParticles)]

        # for i, oldPos in enumerate(self.particles):
        #     self.particles[i] = self.getPositionDistribution(gameState, oldPos).sample()
